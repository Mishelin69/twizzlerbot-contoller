 ...
 */
#include "../include/lvgl_v9_port.h"
...
static void touchpad_read(lv_indev_t *indev_drv, lv_indev_data_t *data)
{
    ...
    esp_lcd_touch_point_data_t touch_point_data[5];
    esp_lcd_touch_get_data(tp, touch_point_data, &touchpad_cnt, 5);

    if (touchpad_cnt > 0) {
        data->point.x = touch_point_data[0].x; // Set the X coordinate
        data->point.y = touch_point_data[0].y; // Set the Y coordinate
        data->state = LV_INDEV_STATE_PRESSED; // Set state to pressed
        ESP_LOGD(TAG, "Touch position: %d,%d", data->point.x, data->point.y); // Log touch position
    } else {
